...
\begin{prop}
	\begin{enumerate}
		...
		\item Soit $(A_{i})_{i \in \mathbb{N}}$ une famille d'évenement finie ou dénombrable deux à deux disjoints tel que $\sum_{i \in \mathbb{N}} P(A_{1}) = 1$. On a\\ $P(B) = \sum_{i \in \mathbb{N}} P(A_{i} \cap B)$ (\textbf{Formule de décomposition})
	\end{enumerate}
\end{prop}
\textit{Preuve.}
\begin{enumerate}
    ...
    \item Montrons que si $A \subset B$, alors $P(A) \leq P(B)$,\\
     ...
     \item On peut écrire $B = B \cap \Omega =  B \cap (\bigcup_{i \in I} A_{i})$. D'après la distributivité on a $B = \bigcup_{i \in I} (B \cap A_{i})$. 
     Comme les $A_{i}$ sont deux à deux disjoints alors ($B \cap A_{i}$) sont deux à deux disjoints. Donc on a $P(B) = P(\bigcup_{i} (B \cap A_{i}) = \sum_{i \in I} P(B \cap A_{i}))$.\\
     D'où $P(B) = \sum_{i \in I} P(B \cap A_{i})$.  \marginpar{$\square$}
\end{enumerate}
...
\begin{enumerate}
       ...
       \item Le nombre d' arragement $A_{n}^{k} = \frac{n!}{(n-k)!}$.
       \item Le coefficient binomial $C_{n}^{k} = \frac{n!}{k!(n-k)!}$.
\end{enumerate}
...
\textbf{Formule de Bayes :} $P(B | A) = \frac{P(A | B) P(B)}{P(A | B)P(B) + P(A | B^{c}) P(B^{c})}$
\end{prop}
