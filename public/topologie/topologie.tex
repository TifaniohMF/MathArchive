...
\begin{defin}[Relation d'ordre]
       Soit X un ensemble non vide. On dit que la relation $\succeq$ est une relation d'ordre si elle vérifie les propriété suivants:
       \begin{itemize}
              \item Reflexivité: pour tout $x \in X, \; x \succeq x$
              \item Anti-symetrie:  pour tout $x, y \in X, x \succeq y, y \succeq x$, alors $x=y$,
              \item Transitivité : pour tout $x,y,z \in X, x \succeq y \mbox{ et } y \succeq z$ alors $x \succeq  z$.
       \end{itemize}
       L'ensemble $(A, \succeq)$ est appelé ensemble ordonnée.
\end{defin}

\begin{defin}[Filtrant à droite]
        On dit que $(A, \succeq)$ est un ensemble dirigé ou un filtrant à droite si pour tout $\alpha, \beta \in (A \times A)$, il existe $\zeta \in A$ tel que $\alpha \succeq \zeta$ et $\beta \succeq \zeta$.
\end{defin}

\begin{defin}[Ensemble cofinal]
      Soit $(A, \preceq)$ un ensemble dirigé. On dit que $B \subseteq A$ est cofinal si pour tout $\alpha \in A$, on peut trouver $\beta \in B$ tel que $\alpha \leq \beta$.
\end{defin}
